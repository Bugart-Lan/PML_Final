...
def get_architectures(arch_file, env_name):
    ...
    # Build hnet_decoder
    hnet_decoder = Mlp(
        input_dim=2 * q_dim, output_dim=1, layer_dims=hnet_decoder_layer_dims
    )
    # Build latent z_encoder
    z_encoder = Encoder(
        input_dim=2 * q_dim,
        share_layer_dims=z_encoder_share_layer_dims,
    ...
    z_decoder = Mlp(
        input_dim=z_dim, output_dim=2 * q_dim, layer_dims=z_decoder_layer_dims
    )
...
def get_train_params(param_file, env_name):
    ...
    control_coef, dynamic_hidden = (
        info["control_coef"].values[0],
        info["dynamic_hidden"].values[0],
    )
    # Get hyperparameter loss function
    alpha1, alpha2, beta1, beta2 = (
        info["alpha1"].values[0],
        info["alpha2"].values[0],
        info["beta1"].values[0],
        info["beta2"].values[0],
    )
    ...
    num_epoch1, num_iter1, batch_size1, lr1, log_interval1 = (
        info["num_epoch1"].values[0],
        info["num_iter1"].values[0],
        info["batch_size1"].values[0],
        info["lr1"].values[0],
        info["log_interval1"].values[0],
    )
    ...
    num_epoch2, num_iter2, batch_size2, lr2, log_interval2 = (
        info["num_epoch2"].values[0],
        info["num_iter2"].values[0],
        info["batch_size2"].values[0],
        info["lr2"].values[0],
        info["log_interval2"].values[0],
    )
...
def save_models_phase2(hnet_decoder, z_encoder, z_decoder, env_name):
    torch.save(
        hnet_decoder.state_dict(), "models/" + env_name + "/hamiltonian_decoder.pth"
    )
...
def load_models_phase2(hnet_decoder, z_encoder, z_decoder, env_name):
    hnet_decoder.load_state_dict(
        torch.load("models/" + env_name + "/hamiltonian_decoder.pth")
    )
...
def _bootstrap(data, n_boot=2000, ci=68):
    ...
    s1 = np.apply_along_axis(stats.scoreatpercentile, 0, b, 50.0 - ci / 2.0)
    s2 = np.apply_along_axis(stats.scoreatpercentile, 0, b, 50.0 + ci / 2.0)
    return (s1, s2)
...
def plot_eval_benchmarks(
    ...
    for i, method in enumerate(methods):
        ...
        _, p2 = _tsplot(
            ax, np.array(time_steps), data, mode, label=method, color=colors[i]
        )
