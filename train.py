...
def get_extreme_samples(
    ...
    with torch.no_grad():
        for q in qs:
            ...
            for i in range(max_num_step):
                if env.criteria_q(qp_traj[i].detach().numpy()[:q_dim]) > env.criteria_q(
                    qp_traj[i + 1].detach().numpy()[:q_dim]
                ):
                    seed_qp = qp_traj[i]
        ...
        for mu, logvar in seed_z:
            ...
            for i in range(num_samples_per_seed):
                ...
                qp = odeint(
                    hd_inverse_net, qp_t, torch.tensor(times, requires_grad=True)
                )[-1]
...
def train_phase_1(
    ...
    optim = torch.optim.Adam(
        list(hnet.parameters()) + list(adj_net.parameters()), lr=lr
    )
    optim.zero_grad()
    ...
    for i in range(num_epoch):
        ...
        for j in range(num_iter):
            ...
            q = q_dat[
                np.random.choice(num_samples, batch_size, replace=False)
            ...
            if (j + 1) % log_interval == 0:
                print(
                    "Average loss for {}th iteration is: {}".format(
                        j + 1, total_loss / cnt
                    )
                )
                total_loss = 0
...
def main_training(
    ...
    retrain_phase1=True,
    retrain_phase2=True,
    num_examples_phase2=0.2,
    num_additional_train=5,
):
    """
    ...
    if mode == 2:
        qs_extreme_np = get_extreme_samples(
            env,
            adj_net,
            hnet,
            hnet_decoder,
...
def train(
    ...
    num_additional_train=5,
    num_examples_phase2=0.2,
    arch_file="models/architectures.csv",
    param_file="models/train_params.csv",
):
    print(f"\nTraining environment {env_name}:\n")
    env = utils.get_environment(env_name)
    _, adj_net, hnet, hnet_decoder, z_encoder, z_decoder = utils.get_architectures(
        arch_file, env_name
    )
    (
        T1,
        ...
        lr2,
        log_interval2,
    ) = utils.get_train_params(param_file, env_name)
    T1, T2 = float(T1), float(T2)

    # Starting point samples
    q_samples = torch.tensor(
        env.sample_q(num_examples, mode="train"), dtype=torch.float
    )
    ...
    main_training(
        ...
        beta1=beta1,
        beta2=beta2,
        num_epoch1=num_epoch1,
        num_iter1=num_iter1,
        batch_size1=batch_size1,
        lr1=lr1,
        log_interval1=log_interval1,
        num_epoch2=num_epoch2,
        num_iter2=num_iter2,
        batch_size2=batch_size2,
        lr2=lr2,
        log_interval2=log_interval2,
        mode=mode,
        retrain_phase1=retrain_phase1,
        retrain_phase2=retrain_phase2,
        num_examples_phase2=num_examples_phase2,
