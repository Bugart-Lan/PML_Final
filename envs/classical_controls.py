...
class ContinuousEnv:
    ...
    def nabla_g(self, q):
        ...
        for i in range(self.q_dim):
            ret[:, i] = (
                self.g(q + self.eps * self.id[i]) - self.g(q - self.eps * self.id[i])
            ) / (2 * self.eps)
        return ret
...
class MountainCar(ContinuousEnv):
    ...
    def f(self, q, u):
        # Noisy forces
        u += np.random.random(1) * 0.4 - 0.2
        force = np.clip(u, self.min_action, self.max_action)
        return np.concatenate(
            (q[:, 1:], force[:, 0:] * self.power - 0.0025 * np.cos(3 * q[:, 0:1])),
            axis=1,
        )
    ...
    def f_u(self, q):
        N = q.shape[0]
        return np.concatenate(
            (np.zeros((N, 1, 1)), np.ones((N, 1, 1)) * self.power), axis=1
        )

    def L(self, q, u, gamma=1e-3):
        return (
            0.1 * np.sum(u**2)
            + self.g(q)
            - gamma
            * (
                np.sum(np.log(-(u - self.max_action)))
                + np.sum(np.log(-q[:, 0] + self.max_position))
                + np.sum(np.log(q[:, 0] - self.min_position))
                + np.sum(np.log(-np.abs(q[:, 1]) + self.max_speed))
            )
        )
    ...
    def w(self, q, epsilon):
        return torch.sum(self.g(q)), torch.sum(
            self.g(q)
            - epsilon
            * (
                torch.log(-(q[:, 0] - self.max_position))
                + torch.log(-(-q[:, 0] + self.min_position))
                # + torch.log(-(torch.abs(q[:, 1]) - self.max_speed))
            )
        )
        ...
        return np.concatenate(
            (
                ...
                * np.random.uniform(
                ...
                np.random.uniform(
                    high=self.max_speed, low=-self.max_speed, size=(num_examples, 1)
                ),
            ),
            axis=1,
        )
...
class CartPole(ContinuousEnv):
    ...
    def f(self, q, u):
        ...
        thetaacc, xacc = (
            thetaacc.reshape(-1, 1),
            xacc.reshape(-1, 1),
        )

        return np.concatenate(
            (x_dot.reshape(-1, 1), xacc, theta_dot.reshape(-1, 1), thetaacc), axis=1
        )
    ...
    def f_u(self, q):
        ...
        return np.concatenate(
            (np.zeros((N, 1)), xacc_u, np.zeros((N, 1)), thetaacc_u), axis=1
...
class Pendulum(ContinuousEnv):
    ...
    def f(self, q, u):
        ...
        return np.concatenate(
            (
                q[:, 1:],
                3 * gravity / (2 * l) * np.sin(q[:, 0:1]) + 3 / (m * (l**2)) * u[:, 0:],
            ),
            axis=1,
        )
    ...
    def f_u(self, q):
        ...
        N = q.shape[0]
        return np.concatenate(
            (np.zeros((N, 1, 1)), 3 / (m * (l**2)) * np.ones((N, 1, 1))), axis=1
        )
