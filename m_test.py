    ...
    if test_trained:
        ...
        if phase2:
            ...
            hnet.load_state_dict(
                torch.load("models/" + env_name + "/hamiltonian_dynamics.pth")
            )
...
def benchmarks(env_name, num_trajs, time_steps):
    # Calculate result
    final_costs_untrained = _test(
        env_name, num_trajs, time_steps, test_trained=False, phase2=False
    )
    final_costs_phase_1 = _test(
        env_name, num_trajs, time_steps, test_trained=True, phase2=False
    )
    final_costs_phase_2 = _test(
        env_name, num_trajs, time_steps, test_trained=True, phase2=True
    )
